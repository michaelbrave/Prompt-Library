"""
Extract 20 Stable Diffusion prompts from blaze.today article.
Produces import-ready JSON for the database.
"""

import re
import json
import datetime

INPUT_FILE = "blaze20_prompts.txt"
OUTPUT_FILE = "seeds/blaze20_import.json"

with open(INPUT_FILE, "r", encoding="utf-8") as f:
    text = f.read()

# Find the section with the numbered prompt list
# Starts after: "Here's a curated selection of stable diffusion prompts"
section = text.split("Here's a curated selection of stable diffusion prompts")[1] \
    if "Here's a curated selection of stable diffusion prompts" in text else text

# Split at "Want your own personal AI copilot" which follows the prompts
section = section.split("Want your own personal AI copilot")[0] \
    if "Want your own personal AI copilot" in section else section

# Extract numbered items from this section only
prompt_pattern = re.compile(r'^\s*(\d+)\.\s*(.+?)$', re.MULTILINE)

entries = []
for m in prompt_pattern.finditer(section):
    num = int(m.group(1))
    if 1 <= num <= 20:
        prompt_text = m.group(2).strip()
        if prompt_text:
            entries.append({
                "num": num,
                "text": prompt_text,
            })

entries.sort(key=lambda e: e["num"])
print(f"Extracted {len(entries)} prompts")

# ── Build import JSON ──────────────────────────────────────────────

def generate_identifier(concept, index):
    words = concept.lower().split()[:4]
    slug = "_".join(re.sub(r'[^a-z0-9]', '', w) for w in words if w)
    if not slug:
        slug = "prompt"
    return f"blaze20_{slug}_{index:03d}"

def generate_concept(prompt_text, max_words=8):
    text = prompt_text.strip()
    text = re.sub(r'--\w+(?:\s+[\w\.\-]+)?', '', text)
    text = re.sub(r'[^a-zA-Z\s]', ' ', text)
    text = re.sub(r'\s+', ' ', text).strip()
    words = text.split()[:max_words]
    return " ".join(words) if words else text[:50]

def detect_syntax(positive):
    sentences = re.split(r'[.!?]+', positive)
    full_sentences = [s.strip() for s in sentences if len(s.strip()) > 20]
    comma_parts = [p.strip() for p in positive.split(",") if p.strip()]
    if len(full_sentences) >= 2:
        return "everyday-speech"
    if len(comma_parts) > len(full_sentences) * 2:
        return "comma-separated"
    return "everyday-speech"

prompt_identifiers = []
for i, p in enumerate(entries):
    prompt_text = p["text"]
    concept = generate_concept(prompt_text)
    identifier = generate_identifier(concept, i)
    syntax_family = detect_syntax(prompt_text)

    prompt_entry = {
        "identifier": identifier,
        "concept": concept,
        "status": "active",
        "metadata": {
            "original_source": "blaze.today - 20 Best Stable Diffusion Prompts",
            "original_prompt": prompt_text,
            "date_added": datetime.datetime.now().isoformat(),
            "source_file": "blaze20_prompts.txt",
            "source_index": i,
            "title": f"Blaze #{p['num']}",
            "platform": "Stable Diffusion",
            "category": "General"
        },
        "wildcard_refs": [],
        "style_variations": [
            {
                "identifier": syntax_family,
                "syntax_family": syntax_family,
                "negative_prompt_strategy": "minimal",
                "positive_template": prompt_text,
                "negative_template": ""
            }
        ]
    }
    prompt_identifiers.append(prompt_entry)

output = {
    "wildcard_library": {},
    "prompt_identifiers": prompt_identifiers
}

with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
    json.dump(output, f, indent=2, ensure_ascii=False)

print(f"Output: {OUTPUT_FILE}")
print(f"Prompts: {len(prompt_identifiers)}")
